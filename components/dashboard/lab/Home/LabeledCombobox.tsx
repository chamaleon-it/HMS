            ...
            {open && (filteredOptions.length > 0 || (text.trim() !== "" && !options.find(o => o.toLowerCase() === text.trim().toLowerCase()))) && (
                ...
                >
                    ...
                    {text.trim() !== "" && !options.find(o => o.toLowerCase() === text.trim().toLowerCase()) && (
                        <button
                            type="button"
                            className="w-full text-left px-2 py-2 mt-1 rounded-lg text-sm text-(--color-synapse-light) bg-blue-50 hover:bg-blue-100 font-medium transition-colors flex items-center gap-2"
                            onMouseDown={(e) => {
                                e.preventDefault(); // Prevent blur
                                commit(text.trim());
                            }}
