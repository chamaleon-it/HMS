...
function VisitBadge({ count }: { count: number }) {
  ...
  if (count === 1) {
    ...
    dotColor = "bg-[var(--color-synapse-dark)]";
  } else if (count === 2) {
    label = "Second Visit";
    ...
    label = "Fourth Visit";
    colorClass = "bg-purple-50 text-[var(--color-synapse-light)] ring-1 ring-purple-200";
    dotColor = "bg-[var(--color-synapse-light)]";
  } else {
  ...
  return (
    <div className="w-full rounded-2xl border border-gray-200 bg-white shadow-sm p-4 md:p-5">
      {new Date(a.date) > new Date() ? (
        ...
        <div className="absolute z-10 -left-[100px] top-0 bottom-0 w-px bg-[var(--color-synapse-light)]" />
      )}
