...
export default function LabResultsPage() {
  ...
  const [dateField, setDateField] = useState<"Reported" | "Collected">(
    "Reported"
  );
  ...
  const doctors = useMemo(
    () => Array.from(new Set(rows.map((r) => r.doctor))),
    [rows]
  );
    ...
    <div className="min-h-[calc(100vh-80px)] w-full bg-gradient-to-b from-white to-slate-50 p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        ...
        <StatCard
          icon={<span className="text-xl">🏥</span>}
          label="Lab"
          value={rows.filter((r) => r.facility === "Lab").length}
          tone="bg-slate-100"
        />
        <StatCard
          icon={<span className="text-xl">🩻</span>}
          label="Imaging"
          value={rows.filter((r) => r.facility === "Imaging").length}
          tone="bg-slate-100"
        />
        <StatCard
          icon={<span className="text-xl">✅</span>}
        ...
        <StatCard
          icon={<span className="text-xl">🚩</span>}
          label="Flagged"
          value={rows.filter((r) => r.status === "Flagged").length}
          tone="bg-amber-100"
        />
      </div>
            ...
            <div className="flex gap-1.5 p-1 bg-gray-100 rounded-xl overflow-x-auto w-fit">
              {[
                ...
                },
                {
                  ...
                  idleClass:
                    "text-emerald-600 ring-transparent hover:bg-emerald-50",
                },
                {
                  label: "Imaging",
                  value: "Imaging" as const,
                  icon: "🩻",
                  activeClass: "bg-indigo-600 text-white ring-indigo-600",
                  idleClass:
            ...
            <div className="flex items-end gap-2">
              ...
              <div className="flex-1 flex flex-col gap-1">
                ...
                <input
                  ...
                  value={dateRange.to ?? ""}
                  onChange={(e) =>
                    setDateRange((v) => ({ ...v, to: e.target.value || null }))
                  }
                  className="h-11 px-3 rounded-xl ring-1 ring-gray-200"
                />
              </div>
            ...
            {REPORT.map((r, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-2 py-3 text-sm text-gray-700">
                    {r.name.map((e) => (
                      <p key={e._id}>{e.type === "Lab" ? "🧪 Lab" : "🩻 Imaging"}</p>
                    ))}
                  </td>
                  <td className="px-2 py-3 text-sm text-gray-700">
                    {r.lab.name}
                  ...
                  <td className="px-2 py-3 text-xs">
                    {r.name.map(
                      (e) =>
                        e.min &&
                        e.max && (
                          <p key={e._id}>{`${e?.min} ${e.unit} to ${e?.max} ${e.unit}`}</p>
                        )
                    )}
                  </td>
                  <td className="px-2 py-3">
          ...
          <div className="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl p-6 overflow-y-auto">
            ...
            <div className="space-y-3">
              ...
              <div className="text-sm text-gray-600">
                {preview.id} • {preview.patientName} ({preview.patientId})
              </div>
              <div className="text-sm">
                <span className="text-gray-500">Facility: </span>
                {preview.facility === "Lab" ? "🧪 Lab" : "🩻 Imaging"}
              </div>
              <div className="text-sm">
              ...
              <div>
                <Chip
                  ...
                  tone={statusTone(preview.status)}
                />
              </div>
              ...
              <button
                onClick={() => {
                  window.location.href = `/lab/${preview.id}`;
                }}
          ...
          <div className="absolute right-0 top-0 h-full w-full max-w-lg bg-white shadow-2xl p-6 overflow-y-auto">
            ...
            <ol className="relative border-s border-gray-200 ps-5 space-y-6">
              <li>
                ...
                <div className="text-sm">
                  <span className="font-medium">{history.collectedAt}</span> —{" "}
                  {history.facility === "Lab"
                    ? "Sample collected"
                    : "Imaging performed"}
                </div>
              </li>
              ...
              <li>
                ...
                <div className="text-sm">
                  ...
                  — Report {history.reportedAt ? "generated" : "pending"}
                </div>
              </li>
            </ol>
            <button
              onClick={() => {
                window.location.href = `/lab/${history.id}/history`;
              }}
                ...
                onClick={() => {
                  if (shareVia === "Copy link")
                    navigator.clipboard?.writeText(
                      `${window.location.origin}/lab/${shareFor.id}`
                    );
                  setShareFor(null);
                  alert(
  ...
  return (
    <th className="text-left px-2 py-3 select-none">
      ...
      >
        {label}
        <span
          className={`text-[10px] ${isActive ? "opacity-100" : "opacity-40"}`}
        >
          {isActive ? (dir === "asc" ? "▲" : "▼") : "↕"}
        </span>
      </button>
