...
export default function LoginForm({
  ...
  const login = handleSubmit(async (data) => {
    try {
      ...
      setUser({ _id, email, username, name, role });
  ...
  return (
    <form className="mt-5 space-y-4" onSubmit={login}>
      <div>
        <label className="block text-slate-700 text-sm mb-1" htmlFor="usernameOrEmail">
          Email or Username
        </label>
        <div
          className={`flex items-center rounded-lg bg-white border ${errors.usernameOrEmail ? "border-red-400" : "border-slate-300"
        ...
        {errors.usernameOrEmail && (
          <p className="mt-1 text-xs text-red-600">{errors.usernameOrEmail.message as string}</p>
        )}
      </div>
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        <button
          ...
          className="text-[var(--brand)] hover:text-[var(--brand-dark)] underline decoration-[rgba(109,40,217,.25)]"
        >
          Forgot password?
        </button>
      </div>
      ...
      <button
        ...
        className="group w-full inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-white font-medium transition active:scale-[.99] focus:outline-none focus:ring-4 focus:ring-[color:var(--brand-soft)] shadow-[0_6px_20px_-6px_rgba(109,40,217,0.6)] hover:shadow-[0_12px_30px_-10px_rgba(109,40,217,0.75)]"
        style={{
