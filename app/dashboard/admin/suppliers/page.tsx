...
const AdminSuppliersPage: React.FC = () => {
  ...
  const { data: suppliers = [], error, isLoading, mutate } = useSWR<Supplier[]>(
    "/suppliers",
    fetcher
  );
    ...
    <AppShell>
      <div className="p-5 min-h-[calc(100vh-67px)]">
        <main className="flex flex-col gap-6">
          <PharmacyHeader
            title="Suppliers"
            subtitle="Manage pharmacy suppliers, procurement accounts & payables"
          >
            <Button
              className="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white shadow-md font-semibold cursor-pointer"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-blue-50/50 p-6 rounded-2xl border border-blue-100 shadow-xs transition-all hover:scale-[1.01]">
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2 mb-2">
                  <Users className="w-4 h-4 text-blue-600/70" />
                  <p className="text-[11px] font-semibold text-blue-800/70 uppercase tracking-widest">
                    Total Suppliers
                  </p>
                </div>
            ...
            <div className="bg-emerald-50/50 p-6 rounded-2xl border border-emerald-100 shadow-xs transition-all hover:scale-[1.01]">
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2 mb-2">
                  <ShoppingBag className="w-4 h-4 text-emerald-600/70" />
                  <p className="text-[11px] font-semibold text-emerald-800/70 uppercase tracking-widest">
                    Total Purchase
                  </p>
                </div>
                <h3 className="text-xl font-bold text-emerald-900 leading-none">
                  {formatINR(stats.totalPurchase)}
                </h3>
              </div>
            </div>

            <div className="bg-sky-50/50 p-6 rounded-2xl border border-sky-100 shadow-xs transition-all hover:scale-[1.01]">
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2 mb-2">
                  <BarChart3 className="w-4 h-4 text-sky-600/70" />
                  <p className="text-[11px] font-semibold text-sky-800/70 uppercase tracking-widest">
                    Purchase Count
                  </p>
                </div>
                <h3 className="text-xl font-bold text-sky-900 leading-none">
                  {stats.totalPurchaseCount}
                </h3>
              </div>
            </div>

            <div className="bg-rose-50/50 p-6 rounded-2xl border border-rose-100 shadow-xs transition-all hover:scale-[1.01]">
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2 mb-2">
                  <CreditCard className="w-4 h-4 text-rose-600/70" />
                  <p className="text-[11px] font-semibold text-rose-800/70 uppercase tracking-widest">
                    Total Due
                  </p>
                </div>
                <h3 className="text-xl font-bold text-rose-900 leading-none">
                ...
                {isLoading ? (
                  ...
                  sortedSuppliers.map((supplier, idx) => (
                    <TableRow
                      ...
                      onClick={() =>
                        router.push(`/dashboard/admin/suppliers/single?id=${supplier._id}`)
                      }
                    >
                      ...
                      <TableCell className="py-3 align-middle text-right font-semibold text-slate-900 pr-4 space-x-2">
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            router.push(
                              `/dashboard/admin/suppliers/single?id=${supplier._id}`
                            );
                          }}
                        >
                          View
