...
export default function List({
  ...
  const handleStatusUpdate = async (id: string, status: string) => {
    try {
      ...
      if (status === "Consulted") {
        router.push(`/dashboard/admin/pharmacy/?mrn=${appointment?.patient?.mrn}&name=${appointment?.patient?.name}&id=${appointment?.patient?._id}&doctor=${appointment?.doctor._id}&#newOrder`);
      }
    } catch (error) {
      console.error(error);
    }
