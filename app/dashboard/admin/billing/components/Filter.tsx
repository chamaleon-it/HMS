      ...
      <div className="flex flex-wrap items-end gap-6">
        ...
        <div className="space-y-2 min-w-[180px]">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <SelectTrigger className="h-10! bg-slate-50/50 border-slate-200 rounded-lg focus:ring-2 focus:ring-[var(--color-cosmo-copper)]/20 transition-all">
                <div className="flex items-center gap-2">
        ...
        <div className="space-y-2 min-w-[180px]">
          ...
          <div className="flex items-center gap-2">
            <Select
              ...
              onValueChange={(value) =>
                setFilter((prev) => ({ ...prev, doctor: value === "all" ? [] : [value], page: 1 }))
              }
            >
              <SelectTrigger className="h-10! bg-slate-50/50 border-slate-200 rounded-lg focus:ring-2 focus:ring-[var(--color-cosmo-copper)]/20 transition-all">
                <div className="flex items-center gap-2">
              ...
              <SelectContent className="rounded-lg border-slate-200 shadow-xl max-h-64">
                <SelectGroup>
                  <SelectLabel className="text-[10px] uppercase tracking-wider text-slate-400">Doctor</SelectLabel>
                  <SelectItem value="all">All Doctors</SelectItem>
                  {doctors.map(doc => (
                    <SelectItem key={doc._id} value={doc.name}>{doc.name}</SelectItem>
                  ))}
                </SelectGroup>
              </SelectContent>
            </Select>
