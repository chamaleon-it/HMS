...
export default function Labs({ labs }: { labs?: LabsDataTypes[] }) {
  // Manage expanded state per report ID (default is collapsed = false)
  const [expandedIds, setExpandedIds] = useState<Record<string, boolean>>({});

  const validLabs = useMemo(() => {
    return (labs || []).filter((l) => getReportTests(l).length > 0);
  }, [labs]);
  ...
  if (!labs || validLabs.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center bg-white rounded-2xl border border-slate-200 shadow-xs">
        ...
        <h2 className="text-base font-bold text-slate-700 mb-0.5">No Lab Results Found</h2>
  ...
  return (
    <div className="space-y-2.5">
      {/* Top Toolbar with Expand / Collapse All Button */}
      <div className="flex items-center justify-between bg-white px-4 py-2.5 rounded-xl border border-slate-200 shadow-xs">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <span className="font-extrabold text-sm text-slate-900">Lab Results</span>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-(--color-synapse-dark) hover:bg-(--color-synapse-dark) border-none">
              ...
              <TableHead className="text-white font-bold w-[32%]">Test</TableHead>
              <TableHead className="text-white font-bold w-[24%]">Value</TableHead>
              <TableHead className="text-white font-bold w-[26%]">Reference</TableHead>
