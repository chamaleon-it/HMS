...
export default function DoctorIPList() {
    ...
    <AppShell>
      <TooltipProvider>
        <div className="p-5 flex flex-col gap-6 min-h-[calc(100vh-67px)]">
          <DoctorHeader title="In-Patients (IP)" subtitle="Manage admitted patients and clinical care">
            <button
                  ...
                  {isLoading ? (
                    ...
                    ipList.map((ip: any) => (
                      <tr key={ip._id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="py-3 px-4">
                          <Link href={`/dashboard/doctor/ip/single?id=${ip._id}`} className="text-(--color-synapse-light) hover:underline font-medium flex items-center gap-1">
