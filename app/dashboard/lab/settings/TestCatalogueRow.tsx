...
export default function TestCatalogueRow({
    ...
    return (
        <TableRow onContextMenu={(e) => { e.preventDefault(); setViewOpen(true); }} className="cursor-context-menu">
            ...
            <TableCell>
                <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${test.type === 'Lab' ? 'bg-blue-50 text-(--color-synapse-light)' : 'bg-purple-50 text-(--color-synapse-light)'}`}>
                    {test.type}
                </span>
            </TableCell>
            <TableCell className="text-slate-500 text-sm max-w-48 whitespace-break-spaces">
