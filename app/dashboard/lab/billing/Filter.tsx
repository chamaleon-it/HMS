...
export default function Filters({ filter, setFilter, billing }: PropsType) {
  ...
  const handleExportCsv = async () => {
    try {
      ...
      exportData.forEach((b: any, index: number) => {
        ...
        const dueVal = Math.max(0, itemsTotal - (paidVal + discountVal));
        ...
        const status = (() => {
          ...
          if (itemsTotal - (paidVal + discountVal) <= 0.01) {
            return "Paid";
          }
          if (paidVal <= 0.01) {
            return "Unpaid";
      ...
      a.href = url;
      a.download = `Lab_Billing_Export_${new Date()
        .toISOString()
        .slice(0, 10)}.csv`;
  ...
  const handleDownloadReport = async () => {
    try {
      ...
      toast.success("Lab billing PDF report downloaded successfully!");
    } catch (err: any) {
