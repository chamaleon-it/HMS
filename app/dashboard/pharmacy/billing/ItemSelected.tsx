      ...
      <div className="flex items-center gap-2">
        <div className="relative w-full">
          <input
            ...
            ref={itemRef}
            className={
              "h-[42px] w-full rounded-lg border border-slate-200 bg-white/70 px-4 text-[13px] font-medium outline-none focus:ring-4 focus:ring-[var(--color-cosmo-copper)]/10 focus:border-[var(--color-cosmo-copper)] dark:border-slate-700 dark:bg-slate-900/50 transition-all shadow-sm"
            }
            value={item ?? ""}
              ...
              {billingItems.map((i, index) => (
                <div
                  ...
                  className={`group flex items-center justify-between gap-3 p-2 rounded-lg cursor-pointer transition-all border ${
                    selectedIndex === index
                      ? "bg-[var(--color-cosmo-copper)]/10 border-[var(--color-cosmo-copper)]/30 ring-1 ring-indigo-200"
                      : "hover:bg-slate-50 border-transparent hover:border-slate-100"
                  }`}
                >
                  <div className="flex items-center gap-3 overflow-hidden flex-1">
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-100 group-hover:bg-[var(--color-cosmo-copper)]/10 transition-colors">
                      <Tag className="h-4 w-4 text-slate-500 group-hover:text-[var(--color-cosmo-copper)]" />
                    </div>
                    <div className="flex flex-col min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded leading-none group-hover:bg-[var(--color-cosmo-copper)]/20/50 group-hover:text-[var(--color-cosmo-copper)] transition-colors">
                          {i.code}
                        </span>
                  ...
                  <div className="flex items-center gap-2 px-1">
                    <div className="text-sm font-bold text-slate-400 group-hover:text-[var(--color-cosmo-copper)] transition-colors mr-2">
                      {formatINR(i.price)}
                    </div>
                    <button
                      type="button"
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 flex h-8 w-8 items-center justify-center rounded-lg hover:bg-[var(--color-cosmo-copper)]/10 text-slate-400 hover:text-[var(--color-cosmo-copper)] transition-all active:scale-90"
                    >
