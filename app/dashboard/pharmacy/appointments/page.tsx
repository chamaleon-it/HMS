...
const APPOINTMENT_STATUSES = [
  ...
  { label: "Observation", value: "Observation", color: "bg-[#FDF6ED] text-(--color-synapse-light)", key: "O" },
      ...
      <div className="p-6 space-y-6 overflow-y-auto flex-1 bg-gray-50/50 custom-scrollbar">
        ...
        <div className="text-center relative">
          ...
          <div className="flex items-center justify-center gap-2 mt-2 flex-wrap text-sm text-gray-500">
            ...
            {visitCount > 0 && <span className="px-2 py-0.5 bg-blue-50 text-(--color-synapse-light) rounded-md border border-blue-100 font-semibold">{visitCount}th Visit</span>}
          </div>
        </div>
            ...
            {APPOINTMENT_STATUSES.map((s) => (
              <button
                ...
                className={`relative group px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${status === s.value
                  ? "ring-2 ring-offset-1 ring-(--color-synapse-light) border-transparent shadow-sm " + s.color
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 space-y-4">
          <div className="flex items-start gap-4">
            <div className="p-2.5 bg-blue-50 text-(--color-synapse-light) rounded-xl shrink-0">
        ...
        {/* Contact Info */}
        {patient.phoneNumber && (
          <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm flex items-center justify-between group cursor-pointer hover:border-synapse-light/30 transition-colors">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center text-gray-500 group-hover:bg-synapse-light/10 group-hover:text-(--color-synapse-light) transition-colors">
          ...
          toast.custom((t) => (
            <div className={`${t.visible ? 'animate-enter' : 'animate-leave'} max-w-md w-full bg-white shadow-lg rounded-xl pointer-events-auto flex ring-1 ring-black ring-opacity-5 overflow-hidden`}>
                  ...
                  <div className="ml-3 flex-1">
                    ...
                    <div className="mt-4 flex gap-3">
                      <button
                        ...
                        className="inline-flex items-center px-3 py-1.5 border border-transparent shadow-xs text-xs font-medium rounded-md text-white bg-(--color-synapse-light) hover:bg-(--color-synapse-light) focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-(--color-synapse-light)"
                      >
                        Book Follow-up
                      </button>
                      <button
                        onClick={() => toast.dismiss(t.id)}
                        className="inline-flex items-center px-3 py-1.5 border border-gray-300 shadow-xs text-xs font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-(--color-synapse-light)"
                      >
                        Dismiss
                      </button>
                    </div>
              ...
              <div className="flex border-l border-gray-200">
                <button
                  onClick={() => toast.dismiss(t.id)}
                  className="w-full border border-transparent rounded-none rounded-r-lg p-4 flex items-center justify-center text-sm font-medium text-(--color-synapse-light) hover:text-(--color-synapse-light) focus:outline-hidden focus:ring-2 focus:ring-(--color-synapse-light)"
                >
        ...
        <div className="shrink-0 mb-4 px-4 sm:px-0 print:hidden">
          ...
          >
            <div className="flex gap-3 items-center">
              ...
              <PrimaryButton onClick={() => router.push("/dashboard/pharmacy/billing#new")} className="bg-linear-to-br from-(--color-synapse-light) to-(--color-synapse-purple) shadow-indigo-200" title="Direct">
                Direct
              </PrimaryButton>
