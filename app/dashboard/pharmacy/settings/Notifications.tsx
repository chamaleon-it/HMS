...
export default function Notifications({
  ...
  useEffect(() => {
    setPayload((prev) => ({
      ...prev,
      whatsapp: profile?.pharmacy?.notifications?.whatsapp ?? false,
      sms: profile?.pharmacy?.notifications?.sms ?? false,
      inApp: profile?.pharmacy?.notifications?.inApp ?? false,
      note: profile?.pharmacy?.notifications?.note ?? "",
    }));
  }, [profile]);
  ...
  const updateNotificationsSettings = async () => {
    try {
      setLoading(true);
      await toast.promise(api.patch("/users/pharmacy/notifications", payload), {
      ...
      <Card className="border border-slate-200 bg-white/90 shadow-sm backdrop-blur-sm rounded-2xl">
        ...
        <CardContent className="space-y-4 text-sm">
          ...
          <div className="flex items-center justify-between pt-2 text-xs text-slate-500">
            ...
            <Button
              size="default"
              className="h-9 gap-2 rounded-full bg-(--color-synapse-dark) px-5 text-sm font-medium text-white shadow-sm hover:bg-(--color-synapse-purple)"
