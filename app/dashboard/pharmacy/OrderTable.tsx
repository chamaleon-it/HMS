              ...
              <TableCell className="py-3 text-right space-x-1 pr-4">
                ...
                {r.status !== "Draft" && r.isDeleted === false && <div className="flex justify-end items-center gap-1">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    className="gap-2 h-8 text-xs text-(--color-synapse-light) border-synapse-light/30 hover:bg-purple-50 hover:text-(--color-synapse-light)"
...
function PriorityBadge({ priority }: { priority: string }) {
  const map: Record<string, string> = {
    ...
    VIP: "bg-[#FDF6ED] text-(--color-synapse-light)",
