...
export default function BulkUpdateTable({ items, lowStockThreshold, onSave }: Props) {
    ...
    const updateItemFields = (id: string, fields: Partial<NewItem>) => {
        setNewItems(prevItems =>
            prevItems.map((item) => {
                if (item.id === id) {
                    ...
                    if ("gst_p" in fields) {
                        const gp = Number(fields.gst_p) || 0;
                        updated.gst_p = gp;
                        updated.sgst_p = gp / 2;
                        updated.cgst_p = gp / 2;
                    } else if ("cgst_p" in fields || "sgst_p" in fields) {
                        updated.gst_p = (Number(updated.cgst_p) || 0) + (Number(updated.sgst_p) || 0);
                    }

                    if ("pack" in fields || "noOfPack" in fields || "schema_free" in fields) {
                    ...
                    const gross = q * r;
                    let discount = Number(updated.dis) || 0;
                    if ("dis" in fields) {
                        discount = Number(fields.dis) || 0;
                        updated.dis = discount;
                        updated.dis_p = gross > 0 ? Number(((discount / gross) * 100).toFixed(2)) : 0;
                    } else if ("dis_p" in fields) {
                        const dp = Number(fields.dis_p) || 0;
                        updated.dis_p = dp;
                        discount = gross * (dp / 100);
                        updated.dis = Number(discount.toFixed(2));
                    } else {
                    ...
                    const taxable = Math.max(0, gross - updated.dis);
    ...
    const totals = useMemo(() => {
        return newItems.reduce((acc, item) => {
            ...
            const dis = Number(item.dis) || (q * r * ((Number(item.dis_p) || 0) / 100));
            ...
            return acc;
        }, { gross: 0, discount: 0, gst: 0, sgst: 0, cgst: 0, schema_amt: 0, total: Number(billDetails.transportCharges) || 0 });
        ...
        <div className="space-y-6 ">
                ...
                <div className="flex gap-8">
                        ...
                        <Select value={selectedSupplierId} onValueChange={setSelectedSupplierId}>
                            ...
                            <SelectContent className="rounded-lg border-slate-200 ">
                                {suppliers.map((s: { _id: string; name: string }) => (
                                    <SelectItem key={s._id} value={s._id} className="rounded-md focus:bg-synapse-light/10 ">{s.name}</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <Input
                            ...
                            className="h-11 bg-slate-50/50 border-slate-200 rounded-lg focus:ring-2 focus:ring-synapse-light/20 transition-all  "
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative ">
                            ...
                            <Input
                                type="number"
                                placeholder="0.00"
                                className="h-11 bg-slate-50/50 border-slate-200 rounded-lg pl-7 focus:ring-2 focus:ring-synapse-light/20 transition-all  text-slate-700 "
                            ...
                            <div className="flex items-center gap-6 ">
                                <label className="flex items-center gap-2 cursor-pointer group ">
                                    <div className="relative flex items-center justify-center ">
                                        ...
                                        />
                                        <div className="w-5 h-5 rounded-full border-2 border-slate-200 peer-checked:border-(--color-synapse-light) peer-checked:border-[6px] transition-all "></div>
                                    </div>
                        ...
                        <label className="flex items-center gap-3 cursor-pointer group bg-slate-50/50 px-4 py-2 rounded-full">
                            <div className="relative flex items-center justify-center ">
                                <input
                                    ...
                                    className="peer w-5 h-5 opacity-0 absolute z-10 cursor-pointer "
                                />
                                <div className="w-5 h-5 rounded border-2 border-slate-200 peer-checked:bg-(--color-synapse-light) peer-checked:border-(--color-synapse-light) transition-all flex items-center justify-center ">
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="bg-(--color-synapse-dark) hover:bg-(--color-synapse-dark) border-none">
                                ...
                                <TableHead className="min-w-37.5 text-[11px] font-semibold uppercase text-white py-2.5 tracking-wider">PRODUCT NAME</TableHead>
                                ...
                                <TableHead className="text-[11px] font-semibold uppercase text-white py-2.5 text-center tracking-wider">QTY</TableHead>
                                <TableHead className="text-[11px] font-semibold uppercase text-white py-2.5 text-center tracking-wider">SCHEMA (FREE)</TableHead>
                                <TableHead className="text-[11px] font-semibold uppercase text-white py-2.5 text-center tracking-wider min-w-21.25">UNITS</TableHead>
                                <TableHead className="text-[11px] font-semibold uppercase text-white py-2.5 text-center tracking-wider min-w-21.25">MRP</TableHead>
                                <TableHead className="text-[11px] font-semibold uppercase text-white py-2.5 text-center tracking-wider min-w-21.25">Rate</TableHead>
                                <TableHead className="text-[11px] font-semibold uppercase text-white py-2.5 text-center tracking-wider min-w-20">DIS(%)</TableHead>
                                <TableHead className="text-[11px] font-semibold uppercase text-white py-2.5 text-center tracking-wider min-w-20">DIS(₹)</TableHead>
                                {gstType === "inclusive" && (
                                    <TableHead className="text-[11px] font-semibold uppercase text-white py-2.5 text-center tracking-wider min-w-20">GST(%)</TableHead>
                                )}
                            ...
                            <AnimatePresence>
                                {newItems.map((item, index) => (
                                    <motion.tr
                                        ...
                                        animate={{ opacity: 1 }}
                                        exit={{ opacity: 0, x: -20 }}
                                        className="hover:bg-synapse-light/10/30 transition-colors border-b border-slate-100 group"
                                    >
                                        <TableCell className="text-center text-xs font-semibold text-slate-300 py-4 group-hover:text-(--color-synapse-light) transition-colors">
                                            {String(index + 1)}
                                        </TableCell>
                                        ...
                                        <TableCell className="p-2"><Input name="batch" className="h-11 text-xs  border-slate-200 bg-white rounded-lg focus:bg-white focus:border-(--color-synapse-light) focus:ring-4 focus:ring-synapse-light/5 transition-all text-center" value={item.batch} onChange={(e) => updateNewItem(item.id, "batch", e.target.value)} onKeyDown={(e) => handleKeyDown(e, item.id, "batch")} /></TableCell>
                                        ...
                                        <TableCell className="p-2">
                                            <Input
                                                type="number"
                                                data-field="pack"
                                                className="h-11 text-sm  border-slate-200 bg-white rounded-lg focus:bg-white focus:border-(--color-synapse-light) focus:ring-4 focus:ring-synapse-light/5 transition-all text-center"
                                                value={item.pack || ""}
                                        ...
                                        <TableCell className="p-2 text-center">
                                            <Input
                                                ...
                                                className="h-11 text-sm font-semibold border-synapse-light/30 bg-synapse-light/10/20 rounded-lg focus:bg-white focus:border-(--color-synapse-light) focus:ring-4 focus:ring-synapse-light/10 transition-all text-center text-(--color-synapse-light)"
                                                value={item.schema_free || ""}
                                                onChange={(e) => updateNewItem(item.id, "schema_free", Number(e.target.value))}
                                                onKeyDown={(e) => handleKeyDown(e, item.id, "schema_free")}
                                            />
                                        </TableCell>
                                        <TableCell className="p-2 text-center">
                                            <div className="text-sm font-bold text-[#1e293b] group-hover:text-(--color-synapse-light) transition-colors">
                                                {item.qty || 0}
                                            </div>
                                        </TableCell>
                                        <TableCell className="p-2">
                                            <Input
                                                ...
                                                className="h-11 text-sm font-semibold border-slate-200 bg-white rounded-lg focus:bg-white focus:border-(--color-synapse-light) focus:ring-4 focus:ring-synapse-light/5 transition-all text-center text-slate-700"
                                        ...
                                        <TableCell className="p-2">
                                            <Input
                                                ...
                                                className="h-11 text-sm font-semibold border-red-100 bg-red-50/10 rounded-lg focus:bg-white focus:border-red-400 focus:ring-4 focus:ring-red-500/10 transition-all text-center text-red-600"
                                                value={item.dis || ""}
                                                onChange={(e) => updateNewItem(item.id, "dis", Number(e.target.value))}
                                                onKeyDown={(e) => handleKeyDown(e, item.id, "dis", gstType === "exclusive", true)}
                                                placeholder="0"
                                            />
                                        </TableCell>
                                        {gstType === "inclusive" && (
                                            <TableCell className="p-2">
                                                <Select
                                                    ...
                                                    onValueChange={(v) => {
                                                        const val = Number(v);
                                                        updateNewItem(item.id, "gst_p", val);
                                                    }}
                                                >
                                                    <SelectTrigger
                                                        data-field="gst_p"
                                                        className="h-11 border-slate-200 bg-white rounded-lg focus:bg-white focus:border-(--color-synapse-light) focus:ring-4 focus:ring-synapse-light/5 transition-all px-3 text-center"
                                                        onKeyDown={(e) => {
                                        ...
                                        <TableCell className="p-2 text-right pr-8">
                                            <div className="text-base font-bold text-[#1e293b] group-hover:text-(--color-synapse-light) transition-colors drop-shadow-sm">
                ...
                <div className="p-6 bg-[#f8fafc] border-t border-slate-200 flex items-center gap-4 justify-between">
                    <div className="flex items-center gap-3">
                        <Button
                            onClick={addNewRow}
                            variant="outline"
                            className="bg-white text-(--color-synapse-light) hover:text-white hover:bg-(--color-synapse-light) border-synapse-light/30 font-semibold text-xs uppercase tracking-widest px-8 h-12 rounded-xl transition-all shadow-sm active:scale-95"
                        >
                            + Add Drug Row
                        </Button>
                        <Button
            ...
            >
                {[
                    ...
                    { label: "SCHEMA TOTAL", value: totals.schema_amt, color: "text-(--color-synapse-light)" },
                ].filter((stat) => gstType === "exclusive" ? stat.label !== "GST PAYABLE" : true).map((stat, i) => (
                    <div key={i} className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 relative overflow-hidden group hover:border-synapse-light/30 transition-all">
                        <div className="absolute top-0 right-0 w-16 h-16 bg-slate-50 -mr-8 -mt-8 rounded-full group-hover:bg-synapse-light/10 transition-colors" />
            ...
            >
                <div className="lg:col-span-8 space-y-3">
                    ...
                    <Textarea
                        ...
                        className="min-h-35 bg-white border-slate-200 rounded-2xl resize-none focus:ring-4 focus:ring-synapse-light/10 transition-all shadow-sm text-sm p-5 font-medium leading-relaxed placeholder:text-slate-300 border-2"
                ...
                <div className="lg:col-span-4 space-y-6">
                    <div className="bg-white p-8 rounded-3xl border-2 border-slate-100 shadow-xl shadow-slate-200/50 space-y-6">
                        <div className="flex justify-between items-center border-b pb-4 border-slate-50">
                            ...
                            <div className="px-3 py-1 bg-synapse-light/10 text-(--color-synapse-light) rounded-full text-[10px] font-bold">DRAFT</div>
                        </div>
                            ...
                            {(() => {
                                ...
                                return (
                                    <>
                                        ...
                                        <div className="bg-(--color-synapse-dark) -mx-8 px-8 py-6 mt-6 shadow-2xl shadow-(--color-synapse-purple)/20">
                        ...
                        <div className="pt-2">
                            ...
                            <div className="relative group">
                                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold group-focus-within:text-(--color-synapse-light) transition-colors">₹</span>
                                <Input
                                    type="number"
                                    placeholder="0.00"
                                    className="h-14 bg-synapse-light/10/40 border-2 border-[var(--color-synapse-light)]/20/50 rounded-2xl pl-10 focus:ring-8 focus:ring-synapse-light/10 focus:border-(--color-synapse-light) transition-all font-black text-xl text-(--color-synapse-light) tabular-nums shadow-sm"
                        ...
                        <Button
                            ...
                            disabled={isSaving}
                            className={cn(
                                "w-full bg-(--color-synapse-light) text-white h-16 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] shadow-2xl shadow-(--color-synapse-light)/30 transition-all active:scale-95 flex items-center justify-center gap-3 border-b-4 border-(--color-synapse-purple)"
                            )}
                        >
                            {isSaving ? (
                                <>
